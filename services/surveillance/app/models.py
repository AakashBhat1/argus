...
class Camera(Base):
    ...
    # Always "surveillance" here; gate and lot cameras live in the parking
    # service. The column (and gate_roi) remain for schema compatibility.
    ...
    # homography_world_points, class_sizes_m}. See argus_vision.geometry.
...
class Alert(Base):
    ...
    id = Column(String(36), primary_key=True, default=generate_uuid)
    # Null for alerts raised by another service (e.g. parking) about a camera
    # this database does not own; see ``source`` / ``source_ref``.
    camera_id = Column(String(36), ForeignKey("cameras.id"), nullable=True)
    tenant_id = Column(String(36), default="1", index=True)
    # Originating service ("surveillance", "parking", ...) and its alert id.
    source = Column(String(32), default="surveillance", nullable=False)
