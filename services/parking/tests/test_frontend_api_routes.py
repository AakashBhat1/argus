...
def test_frontend_parking_routes_match_parking_openapi():
    repo_root = Path(__file__).resolve().parents[3]
    api_ts_path = repo_root / "frontend" / "src" / "lib" / "api.ts"
    endpoints = [
        ep for ep in extract_api_ts_endpoints(api_ts_path) if ep.startswith(PARKING_PREFIXES)
    ]
    assert len(endpoints) >= 10, f"Expected parking endpoints in api.ts, got {len(endpoints)}"
    missing_routes = validate_endpoints_against_openapi(endpoints)
    assert not missing_routes, f"Frontend parking paths missing from the parking API: {missing_routes}"
